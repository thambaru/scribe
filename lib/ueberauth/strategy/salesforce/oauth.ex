defmodule Ueberauth.Strategy.Salesforce.OAuth do
  ...
  The response includes `instance_url` which is required for API calls.
  """
  ...
  @doc """
  Fetches user identity info from Salesforce using the id URL from the token.

  Returns organization_id, user_id, and other user details.
  """
  def get_user_info(access_token, id_url) do
    headers = [{"Authorization", "Bearer #{access_token}"}]

    case Tesla.get(http_client(), id_url, headers: headers) do
